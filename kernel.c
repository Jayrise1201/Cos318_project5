...
static void create_segment(struct segment_t *entry, uint32_t base,
         uint32_t limit, char type, char privilege,
         char seg_type);
...
void select_page_directory(void)
{
  asm volatile ("movl %%eax, %%cr3 "::
          "a" (current_running->page_directory));
}
...
void create_segment(struct segment_t *entry,  /* GDT entry */
        ...
        /* segment type (system, code, data, stack segment) */
        char type, char privilege /* privilege level */,
 ...
 * 5.  Read GDT[ IDT[entry].selector ], to obtain the code segment start
 *     address and size.
...
static void init_idt(void)
{
  ...
  create_gate(&(idt[IRQ_START + 5]),
        (uint32_t) pci5_entry, KERNEL_CS, INTERRUPT_GATE, 0);
  create_gate(&(idt[IRQ_START + 9]),
        (uint32_t) pci9_entry, KERNEL_CS, INTERRUPT_GATE, 0);
  create_gate(&(idt[IRQ_START + 10]),
        (uint32_t) pci10_entry, KERNEL_CS, INTERRUPT_GATE, 0);
  create_gate(&(idt[IRQ_START + 11]),
        (uint32_t) pci11_entry, KERNEL_CS, INTERRUPT_GATE, 0);
  ...
  create_gate(&(idt[IDT_SYSCALL_POS]),
        (uint32_t) syscall_entry, KERNEL_CS, INTERRUPT_GATE, 3);
...
void init_gdt(void)
{
  ...
  create_segment(gdt + KERNEL_DATA, 0, 0xfffff, DATA_SEGMENT, 0,  
           MEMORY);

  /* Code segment for processes */
  create_segment(gdt + PROCESS_CODE, 0, 0xfffff, CODE_SEGMENT, 3,
           MEMORY);

  /* Data segment for processes (contains user stack) */
  create_segment(gdt + PROCESS_DATA, 0, 0xfffff, DATA_SEGMENT, 3,
           MEMORY);

  /* Insert pointer to the global TSS */
  create_segment(gdt + TSS_INDEX, (uint32_t) & tss, TSS_SIZE,
           TSS_SEGMENT, 0,
           SYSTEM);
...
void init_tss(void)
{
  ...
  /*
   * Load the index in the GDT corresponding to the TSS into the
   * Task Register (p 153 PMSA).
   */
...
static int create_thread(int i)
{
  ...
  /*
   * Enable interrupts if the IF bit in the indicated EFlags
   * value is set.
   */
  STI_FL(eflags);
  ...
  /*
   * All threads run in kernel address space, the start address
   * is known
   */
...
void print_status(int time)
{
  ...
  scrprintf(base - 3, 0, "%-5s%-10s%-10s%-10s%-10s%-10s%-10s%-10s",
    "Pid", "Type", "Status", "Disable", "Preempt", "Yield", "Page",   "Kernel");
  scrprintf(base - 2, 0, "%-5s%-10s%-10s%-10s%-10s%-10s%-10s%-10s",
    "",     "",    "",       "count",   "count",   "",      "faults", "stack");
  enter_critical();
  p = current_running;
  i = 0;
  do {
    scrprintf(base + i, 0, "%-5d%-10s%-10s%-10d%-10d%-10d%-10d%-10x",
      p->pid,
...
int readdir(unsigned char *buf)
{
  ...
  /* read the boot block */
  scrprintf(23, 0, "reading bootblock");
  ...
  scrprintf(23, 0, "reading directory");
